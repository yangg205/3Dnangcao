...
public class Dangky : MonoBehaviour
{
    ...
    private void Start()
    {
        HideNotification(); // Ẩn thông báo khi khởi động
    }
    ...
    public void OnRegisterClick()
    {
        ...
        {
            ShowNotification("Tên, email và mật khẩu không được để trống.");
            return;
        }
        ...
        StartCoroutine(DangKy(json));
    }

    // Coroutine thực hiện gọi API đăng ký
    IEnumerator DangKy(string json)
    {
        var url = "http://yang2206-001-site1.ptempurl.com/api/regiter"; // API URL
        using (UnityWebRequest request = new UnityWebRequest(url, "POST"))
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    Debug.LogError($"Lỗi đăng ký: {response?.data?.message}");
                }
            }
        }
    }
    ...
    private void ShowNotification(string message)
    {
        ...
        StartCoroutine(HideNotificationAfterDelay(5f)); // Tự động ẩn sau 5 giây
    }
